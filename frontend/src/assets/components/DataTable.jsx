...
const DataTable = ({ activeDashboard, excelData, filteredCompanies, handleDeleteCompany, handleEdit }) => {
  ...
  const fetchComments = async (cdrId) => {
    setIsLoading(true);
    try {
      console.log(`Fetching comments for uniqueid: ${cdrId}`);
      const response = await defaultInstance.get(`/comments/${cdrId}`);
      ...
      setComments([]);
      alert('Не удалось загрузить комментарии: ' + (error?.response?.data?.error || error.message));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const openCommentModal = (cdrId) => {
    ...
    console.log('Opening comment modal for uniqueid:', cdrId);
    setCurrentCdrId(cdrId);
    setComments([]); // Сбрасываем перед загрузкой
    fetchComments(cdrId);
  ...
  const saveComment = async () => {
    ...
    try {
      console.log('Saving comment:', { uniqueid: currentCdrId, comment: newComment });
      ...
      alert('Не удалось сохранить комментарий: ' + (error?.response?.data?.error || error.message));
    }
  };
          ...
          <div className="row">
            <div className="col-12">
              <div key={activeDashboard} className="animated-section fade-in">
                <div className="card">
                  <h5 className="card-header">{t('recentOrders')}</h5>
                      ...
                      <table className="table">
                        <thead className="bg-light">
                          <tr className="border-0">
                            ...
                            <th>{t('totalValueGorgia')}</th>
                            <th>{t('lastPurchaseDateGorgia')}</th>
                            <th>{t('contractEndDate')}</th>
                          ...
                          {dataToDisplay.length > 0 ? (
                            dataToDisplay.map((company, index) => (
                              <tr key={company.id || index}>
                                ...
                                <td>{company.phone3 || company.phone3 || 'N/A'}</td>
                              ...
                              <tr key={index}>
                                ...
                                <td>
                                  ...
                                  <button
                                    ...
                                    className={edit_delete.commentButton}
                                  >
                                    <svg className={edit_delete.commentSvgIcon} viewBox="0 0 512 512">
                                      <path d="M512 240c0 114.9-114.6 208-256 208-37.1 0-72.3-6.4-104.1-17.9-11.9 8.7-31.3 20.6-54.3 30.6C73.6 471.1 44.7 480 16 480c-6.5 0-12.3-3.9-14.8-9.9-2.5-6-1.1-12.8 3.4-17.4 15.3-15.8 35.2-33.2 51.1-45.4C24.2 368.4 0 310.2 0 240 0 125.1 114.6 32 256 32s256 93.1 256 208z"/>
                                    </svg>
                                  </button>
                                </td>
                              </tr>
